    ...
    $.fn.extend({
        slimScroll: function (options) {
            ...
            this.each(function () {
                ...
                if (me.parent().hasClass(o.wrapperClass) && $.inArray(o.barClass, me.parent().children().map(function (i, el) {
                    return $(el).attr('class');
                })) >= 0 && $.inArray(o.railClass, me.parent().children().map(function (i, el) {
                    return $(el).attr('class');
                })) >= 0) {
                ...
                if (o.railDraggable) {
                    bar.bind("mousedown", function (e) {
                        ...
                        $doc.bind("mousemove.slimscroll", function (e) {
                            ...
                            bar.css('top', currTop);
                            if (parseInt(o.scrollDelay) > 0) {
                                if (timeout) {
                                    clearTimeout(timeout);
                                }
                                timeout = setTimeout(function () {
                                    scrollContent(0, bar.position().top, false);
                                }, parseInt(o.scrollDelay));
                            } else {
                                scrollContent(0, bar.position().top, false);
                            }
                        });
                ...
                // rail clickable
                rail.on('mousedown.slimscroll', function (e) {
                    var pageY = e.pageY;
                    scrollContent(o.wheelStep * 5 * (pageY > bar.offset().top + bar.height() ? 1 : -1), false, false, true);
                });
                ...
                function _onWheel(e) {
                    ...
                    if (e.wheelDeltaY || e.deltaY || e.axis == 2) {
                        ...
                        if ($(target).closest('.' + o.wrapperClass).is(me.parent())) {
                            // scroll content
                            scrollContent(delta, true, false, true);
                        }
                ...
                function scrollContent(y, isTop, isJump, isWheel) {
                    releaseScroll = false;
                    var delta = y;
                    ...
                    if (isWheel) {
                        ...
                        var top = me.scrollTop(), maxMeTop = me[0].scrollHeight - me.outerHeight();
                        delta = top + y * parseInt(o.wheelStep);
                        delta = Math.min(Math.max(delta, 0), maxMeTop);
                        // calculate a new bar position
                        top = maxTop * delta / maxMeTop;
